...
SpritePixels generateFloorBorderOverlay(uint32_t seed, uint8_t style, uint8_t mask, int variant, int frame, int pxSize = 16);
// Isometric floor border overlay (diamond, transparent).
// Generated by projecting the top-down border overlay into 2:1 diamond space.
SpritePixels generateIsometricFloorBorderOverlay(uint32_t seed, uint8_t style, uint8_t mask, int variant, int frame, int pxSize = 16);
